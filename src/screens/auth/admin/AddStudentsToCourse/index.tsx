...
import { useDataAddStudents } from './hooks/useDataAddStudents'
...
type AddStudentScreenProps = RouteProp<RootStackParamListAdmin, 'AddStudentsToCourse'>;
...
export default function AddStudentsToCourse() {
  ...
  const {
    ...
    selectedStudents,
    setSelectedStudents
  } = useDataAddStudents(params.courseId);
            ...
            onPress: () => {
              (async () => {
                const course_student_list: CourseStudentRequest[] = selectedStudents.map(student => {
                  return ({ course_id: params.courseId, user_id: student.user_id });
                });
